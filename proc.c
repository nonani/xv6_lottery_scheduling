...
int clone(void (*fcn)(void *, void *), void *arg1, void *arg2, void *stack)
{
  ...
  if (((uint)stack % PGSIZE) != 0)
    return -1;
  if ((myproc()->sz - (uint)stack) < PGSIZE)
    return -1;
  if ((np = allocproc()) == 0)
    return -1;

  np->pgdir = cur_proc->pgdir;
  ...
  *np->tf = *cur_proc->tf;
  ...
  for (i = 0; i < NOFILE; i++)
    if (cur_proc->ofile[i])
      np->ofile[i] = filedup(cur_proc->ofile[i]);
  np->cwd = idup(cur_proc->cwd);

  pid = np->pid;
  np->state = RUNNABLE;
  safestrcpy(np->name, cur_proc->name, sizeof(cur_proc->name));
  np->thread_count = ++cur_proc->thread_count;
  return pid;
}
